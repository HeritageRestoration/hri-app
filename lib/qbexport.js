...
function fmtDate(d) {
  if (!d) return 'N/A';
  try {
    // PostgreSQL may return a Date object or a string like "2026-05-16T00:00:00.000Z"
...
async function buildExportRows(weekStart) {
  ...
  for (const tc of timecards) {
    ...
    for (const r of (detail.rows||[])) {
      ...
      const payType = r.section === 'boardup' ? 'Board-Up' : r.is_office ? 'Admin/Office' : r.is_nonbill ? 'Non-Billable' : 'Regular';
