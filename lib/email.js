...
// Single condensed admin summary — used for both Friday and Monday
function emailAdminSummary(names, timing, appUrl) {
  const isFriday = timing === 'friday';
  const title    = isFriday ? 'Friday Reminder — Time Cards Not Yet Submitted' : 'Monday Follow-Up — Time Cards Overdue';
  const intro    = isFriday
    ? `It's Friday evening. The following ${names.length} employee${names.length > 1 ? 's have' : ' has'} <strong>not yet submitted</strong> a time card for this week:`
    : `Monday morning follow-up. The following ${names.length} employee${names.length > 1 ? 's have' : ' has'} <strong>still not submitted</strong> their time card from last week:`;
  const color    = isFriday ? '#92400e' : '#991b1b';
  const bg       = isFriday ? '#fffbf0' : '#fff5f5';
  const list = names.map(n =>
    `<li style="padding:4px 0;border-bottom:0.5px solid #e5e7eb;font-size:13px">${n}</li>`
  ).join('');
  return wrap(title, `
    <p style="margin-bottom:12px;color:${color}">${intro}</p>
    <ul style="margin:0 0 16px;padding:10px 10px 10px 30px;background:${bg};border-radius:6px;border:0.5px solid #e5e7eb;list-style:disc">${list}</ul>
    <p style="font-size:12px;color:#4a5e78;margin-bottom:12px">${isFriday
      ? 'Individual reminder emails have been sent directly to each employee.'
      : 'Individual overdue notices have been sent directly to each employee. Please follow up as needed for payroll processing.'
    }</p>
    <a href="${appUrl}/admin" style="background:#1a2e4a;color:#fff;padding:10px 20px;border-radius:7px;text-decoration:none;font-weight:600;font-size:13px;display:inline-block">View Admin Dashboard →</a>
  `);
}
...
module.exports = { sendMail, emailSubmitted, emailFridayReminder, emailMondayReminder, emailMondayReminderEmployee, emailAdminSummary };
