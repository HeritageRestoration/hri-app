...
async function getGraphToken() {
  ...
  return new Promise((res, rej) => {
    const req = https.request({
      ...
      path: `/${tid}/oauth2/v2.0/token`, method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Content-Length': Buffer.byteLength(body) }
    }, r => {
      let d = ''; r.on('data', c => d += c);
      r.on('end', () => { const p = JSON.parse(d); p.access_token ? res(p.access_token) : rej(new Error(p.error_description||d)); });
    });
...
async function sendMail(to, subject, html, cc) {
  ...
  const payload = JSON.stringify({
    message: {
      ...
      ccRecipients: cc ? [].concat(cc).map(a => ({ emailAddress: { address: a } })) : []
    },
  ...
  await new Promise((res, rej) => {
    const req = https.request({
      ...
      path: `/v1.0/users/${encodeURIComponent(process.env.MS_SENDER_EMAIL)}/sendMail`,
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(payload) }
    }, r => {
      let d = ''; r.on('data', c => d += c);
      r.on('end', () => (r.statusCode === 202 || r.statusCode === 200) ? res() : rej(new Error(`Graph ${r.statusCode}: ${d}`)));
    });
